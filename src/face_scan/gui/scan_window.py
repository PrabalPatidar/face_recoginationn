"""
Scan window for the GUI application.
"""
...
class ScanWindow:
    """Scan window for face detection and recognition."""
    ...
    def create_widgets(self):
        ...
        # Title
        title_label = ttk.Label(main_frame, text="Face Scan", 
                               font=("Arial", 16, "bold"))
    ...
    def scan_image(self):
        ...
        thread = threading.Thread(target=self._scan_image_thread)
        thread.daemon = True
        thread.start()
    
    def _scan_image_thread(self):
        """Scan image in separate thread."""
        try:
            # Update detector method
            self.face_detector = FaceDetector(self.detection_method.get())
            
            if self.scan_type.get() == "detect":
                self._detect_faces()
            else:
